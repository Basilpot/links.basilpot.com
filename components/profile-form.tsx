...
export function ProfileForm({ profile, availableThemes, platforms }: {
  ...
  return <form action={action} className="space-y-4">
    ...
    <div><label className="label" htmlFor="theme">Theme</label><select id="theme" name="theme" className="input" defaultValue={profile.theme} aria-invalid={!!errors.theme} aria-describedby={errors.theme ? "theme-error" : undefined}>{availableThemes.map(theme => <option value={theme} key={theme}>{theme[0].toUpperCase() + theme.slice(1)}</option>)}</select><FieldError id="theme" message={errors.theme}/></div>
