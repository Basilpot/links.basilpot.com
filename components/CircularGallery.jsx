...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    const currentFont = font ?? `700 24px ${styles.fontFamily}`;
    const currentColor = textColor ?? styles.color;
    resolveFont(currentFont, fontUrl).then(resolvedFont => {
  ...
  return (
    ...
    >
      <ul className="sr-only">{items?.map(item => <li key={`${item.image}-${item.text}`}>{item.text}</li>)}</ul>
    </div>
  );
}
