...
Set `SUPABASE_S3_ACCESS_KEY_ID` and `SUPABASE_S3_SECRET_ACCESS_KEY` on server. Supabase project `amkaxkwibayijmsjuioe` uses S3 region `ap-southeast-2`. Use public `links-by-basilpot` bucket with PNG, JPEG, and WebP uploads up to 4 MB. Profile and link images are uploaded through authenticated app actions; database stores only object paths. Add same two secrets to deployment environment.
